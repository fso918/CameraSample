...
public class AntiNGraph extends GameGraph {
    ...
    public boolean canTurnShape(GameMap map) {
        ...
        switch (state){
            case STATE_TURN_1:
                if(GameUtils.isBlocked(map, x - 1, y) || GameUtils.isBlocked(map, x + 1, y - 1)){
                    canTurnShape = false;
                }
                break;
            case STATE_TURN_2:
    ...
    public static GameGraph getRandomIns(){
        return new AntiNGraph(new Random().nextInt(2) + 1);
    }
}
